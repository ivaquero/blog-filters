...
def plot_belief_prior(belief, prior, ylim, figwidth=8):
    _, axes = plt.subplots(
        1, 2, figsize=(figwidth, figwidth / 2.5), constrained_layout=True, sharey=True
    )
    for data, title, ax in zip(
        [belief, prior], ["Belief", "Prior"], axes.flatten(), strict=True
    ):
...
def plot_prior_posterior(prior, posterior, ylim, figwidth=8):
    ...
    for data, title, ax in zip(
        [prior, posterior], ["Prior", "Posterior"], axes.flatten(), strict=True
    ):
        ax.bar(range(len(data)), data)
        ax.set(title=title, ylim=ylim)
        ax.grid(1)
